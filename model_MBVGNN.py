...
class Model(object):
	...
	def _process_batch(self, input, targets, phase):
		...
		graph_global = input.graph_global.to(self.device)
		targets = targets.to(self.device)
		self.optimizer.zero_grad()

		with torch.set_grad_enabled(phase == 'train'):
			outputs = self.model(nodes, edge_sources, edge_targets, edge_distance, graph_indices, node_counts,
								 combine_sets,graph_global,plane_wave)
		...
		for input, targets in dataloader:
			...
			with torch.set_grad_enabled(False):
				outputs, graph_vec = self.model(nodes, edge_sources, edge_targets, edge_distance, graph_indices,
												node_counts, combine_sets, graph_global,plane_wave,output_graph=True)
...
def _bn_act(num_features, activation, use_batch_norm=True):
	if use_batch_norm:
...
class GatedGraphConvolution(Module):
	def __init__(self, n_node_feat, in_features, out_features, n_grid_K,n_Gaussian,
		...
		self.linear_gate = Linear(in_features+1, out_features, bias=bias)
		self.activation_gate = _bn_act(out_features, gate_activation, use_edge_batch_norm)

		self.linear_MLP = Linear(in_features+1, out_features, bias=bias)
		self.activation_MLP = _bn_act(out_features, MLP_activation, use_edge_batch_norm)

	def forward(self, graph_indices, input, nodes, edge_sources, edge_targets, rij, combine_sets, graph_global,plane_wave,cutoff):
		output = input.clone()
		gg = graph_global[graph_indices].unsqueeze(1)

		gg = gg[edge_sources].contiguous()
		...
		final_fe = torch.cat([ni, nj, delta,gg], dim=1)
		del ni, nj, delta
...
class MBVGNN(nn.Module):
	def __init__(self, n_node_feat, n_hidden_feat, n_GCN_feat, conv_bias, N_block, node_activation, MLP_activation,
				 use_node_batch_norm, use_edge_batch_norm, cutoff, max_nei, n_MLP_LR, n_grid_K,n_Gaussian):

		super(MBVGNN, self).__init__()
		self.cutoff = cutoff
		...
		self.conv = [
			GatedGraphConvolution(n_node_feat, n2v_concatent_feat, n_hidden_feat, n_grid_K,n_Gaussian,
		...
		self.MLP_psi2n = [
			OLP(n_hidden_feat, n_hidden_feat, activation=MLP_activation, use_batch_norm=use_node_batch_norm,
				bias=conv_bias)]
		...
		self.MLP_psi2n = [
			OLP(n_hidden_feat, n_hidden_feat, activation=MLP_activation, use_batch_norm=use_node_batch_norm,
				bias=conv_bias) for _ in range(N_block)]
		self.MLP_psi2n = ModuleList(self.MLP_psi2n)

		# gated pooling for every block
		self.gated_pooling = [
			Gated_pooling(n_hidden_feat, n_GCN_feat, activation=MLP_activation, use_batch_norm=use_node_batch_norm,
						  bias=conv_bias) for _ in range(N_block)]
		self.gated_pooling = ModuleList(self.gated_pooling)

		# final linear regression
		self.linear_regression = [
			OLP(int(n_GCN_feat / 2 ** (i - 1)), int(n_GCN_feat / 2 ** i), activation=MLP_activation,
				use_batch_norm=use_node_batch_norm, bias=conv_bias) for i in range(1, n_MLP_LR)]
		self.linear_regression += [
			OLP(int(n_GCN_feat / 2 ** (n_MLP_LR - 1)), 1, activation=None, use_batch_norm=None, bias=conv_bias)]
		self.linear_regression = ModuleList(self.linear_regression)

	def forward(self, nodes, edge_sources, edge_targets, edge_distance, graph_indices, node_counts, combine_sets,graph_global,plane_wave,
				output_graph=False):
		x = self.embedding(nodes)
		Poolingresults = []

		for i in range(self.N_block):
			x = self.conv[i](graph_indices, x, nodes, edge_sources, edge_targets, edge_distance, combine_sets,graph_global,plane_wave,self.cutoff)

			pool = self.gated_pooling[i](x, graph_indices, node_counts)
			Poolingresults.append(pool)

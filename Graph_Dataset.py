...
def Atomgraph_collate(batch):
    ...
    for i, (graph, target) in enumerate(batch):
        ...
        graph_global.append(graph.graph_global)
    ...
    graph_global = np.concatenate(graph_global, axis=0) # 我也不知道有什么用，加了看看！！！！
    input = CDGNN_Input(nodes, edge_distance, edge_sources, edge_targets, graph_indices, node_counts, combine_sets,plane_wave, graph_global)
    targets = torch.Tensor(targets)
    return input, targets


class Graph(object):
    def __init__(self, graph, cutoff, n_grid_K,n_Gaussian):
        lattice,self.nodes, neighbors, graph_globals,volume = graph
        nei = neighbors[0]
        ...
        self.graph_global = np.array(graph_globals, dtype=np.float32)
        combine_sets = []
        # gaussian radial
        N = n_Gaussian
...
class GraphDataset(Dataset):
    def __init__(self, path, filename, database, target_name, cutoff, n_grid_K,n_Gaussian):
        ...
        for i, name in enumerate(self.graph_names):
            graph_data.append(Graph(self.graph_data[name], cutoff, n_grid_K,n_Gaussian))
            if i % 500 == 0 and i > 0:
...
class CDGNN_Input(object):
    def __init__(self, nodes, edge_distance, edge_sources, edge_targets, graph_indices, node_counts, combine_sets,plane_wave, graph_global):
        ...
        self.graph_global = torch.Tensor(graph_global)
